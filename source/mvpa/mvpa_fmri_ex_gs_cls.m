...
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.mvpa_fmri_ex_gs_cls]);
    eval(['! rm -rf ',proj.path.mvpa_fmri_ex_gs_cls]);
    disp(['Creating ',proj.path.mvpa_fmri_ex_gs_cls]);
    eval(['! mkdir ',proj.path.mvpa_fmri_ex_gs_cls]);
end
...
for i = 1:numel(subjs)
    ...
    if(qlty.ok)
        ...
        %% ----------------------------------------
        %% Save out results
        save([proj.path.mvpa_fmri_ex_gs_cls,subj_study,'_',name,'_prds.mat'],'prds');
