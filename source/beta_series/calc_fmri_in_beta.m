...
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.fmri_in_beta]);
    eval(['! rm -rf ',proj.path.fmri_in_beta]);
    disp(['Creating ',proj.path.fmri_in_beta]);
    eval(['! mkdir ',proj.path.fmri_in_beta]);
end
...
for i=1:numel(subjs)
    ...
    dlmwrite([proj.path.code,'tmp/target_path.txt'],proj.path.trg_in,'');
    dlmwrite([proj.path.code,'tmp/project_name.txt'],proj.path.name,'');
    dlmwrite([proj.path.code,'tmp/out_path.txt'],proj.path.fmri_in_beta,'');
