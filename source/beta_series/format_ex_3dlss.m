...
if(proj.flag.clean_build)
    disp(['Removing ',proj.path.trg]);
    eval(['! rm -rf ',proj.path.trg]);
    disp(['Creating ',proj.path.trg]);
    eval(['! mkdir ',proj.path.trg]);
end
...
% concatenate the ids
run1_stim_ids = [repmat(proj.param.ex_id,numel(run1_ex_stim_times),1);
                 repmat(proj.param.in_id,numel(run1_in_stim_times),1);
                 repmat(proj.param.feel_id,numel(run1_feel_stim_times),1)];

% concatenate the valence scores
run1_stim_v_scores = [run1_design.ex_valence_seq';
                 run1_design.in_valence_seq';
                 repmat(proj.param.dummy_score,numel(run1_feel_stim_times),1)];
...
run1_stim_v_labs(find(run1_stim_v_scores>=proj.param.mid_score)) = proj.param.pos_class;
run1_stim_v_labs(find(run1_stim_v_scores<proj.param.mid_score)) = proj.param.neg_class;
run1_stim_v_labs(find(run1_stim_ids==proj.param.feel_id)) = 0; %0 means N/A

% assign binary labels to the arousal stimuli
run1_stim_a_labs = 0*run1_stim_a_scores; % allocate zero vector
run1_stim_a_labs(find(run1_stim_a_scores>=proj.param.mid_score)) = proj.param.pos_class;
run1_stim_a_labs(find(run1_stim_a_scores<proj.param.mid_score)) = proj.param.neg_class;
run1_stim_a_labs(find(run1_stim_ids==proj.param.feel_id)) = 0; %0 means N/A
...
% concatenate the ids
run2_stim_ids = [repmat(proj.param.ex_id,numel(run2_ex_stim_times),1);
                 repmat(proj.param.in_id,numel(run2_in_stim_times),1);
                 repmat(proj.param.feel_id,numel(run2_feel_stim_times),1)];

% concatenate the valence scores
run2_stim_v_scores = [run2_design.ex_valence_seq';
                 run2_design.in_valence_seq';
                 repmat(proj.param.dummy_score,numel(run2_feel_stim_times),1)];
...
run2_stim_v_labs(find(run2_stim_v_scores>=proj.param.mid_score)) = proj.param.pos_class;
run2_stim_v_labs(find(run2_stim_v_scores<proj.param.mid_score)) = proj.param.neg_class;
run2_stim_v_labs(find(run2_stim_ids==proj.param.feel_id)) = 0; %0 means N/A

% assign binary labels to the arousal stimuli
run2_stim_a_labs = 0*run2_stim_a_scores; % allocate zero vector
run2_stim_a_labs(find(run2_stim_a_scores>=proj.param.mid_score)) = proj.param.pos_class;
run2_stim_a_labs(find(run2_stim_a_scores<proj.param.mid_score)) = proj.param.neg_class;
run2_stim_a_labs(find(run2_stim_ids==proj.param.feel_id)) = 0; %0 means N/A
...
filename = [proj.path.trg,'stim_times.1D'];
...
save([proj.path.trg,'stim_ids.txt'],'stim_ids','-ascii');
save([proj.path.trg,'stim_v_scores.txt'],'stim_v_scores','-ascii');
save([proj.path.trg,'stim_a_scores.txt'],'stim_a_scores','-ascii');
save([proj.path.trg,'stim_v_labs.txt'],'stim_v_labs','-ascii');
save([proj.path.trg,'stim_a_labs.txt'],'stim_a_labs','-ascii');
